...
/*
 Synchronized
 	다중 스레드 환경에서 동시 접근으로부터 데이터의 일관성을 보하하기 위해 사용되는 키워드
...
public class SyncMethod {
	private int count = 0;
	private Object lockObj = new Object(); // 동기화를 위한
	...
	/*
	 ...
	 synchronized(lockObj)  코드 블록 내에서 count 변수를 증가시키지 전에 동기화를 수행
	 						lockObj를 사용하여 동기화를 수행
	 lockObj 사용이유 : 다른 메서드에서 동기화에 사용할 수 있고, 
	 			     이렇게 하면 다른 객체에 대한 동기화화 분리되어 충돌 방지 가능
	 */
